...
class Hospital_user_model(models.Model):
    ...
    hospital = models.ForeignKey(Hospital_model, on_delete=models.CASCADE, related_name='hostpitals')
    name = models.CharField(
        max_length=255,
        unique=True
    )
...
class Patient_model(models.Model):
    ...
    hospital = models.ForeignKey(Hospital_model, on_delete=models.CASCADE, related_name='patients')
...
class HospitalUploadLog(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    hospital = models.ForeignKey('Hospital_model', on_delete=models.CASCADE, related_name='upload_logs')
...
class EscalationModel(models.Model):
    ...
    patient = models.ForeignKey(
        'Patient_model',  # Adjust app/model name as needed
        on_delete=models.CASCADE
    )
...
class Patient_date_model(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    hospital = models.ForeignKey(Hospital_model, on_delete=models.CASCADE, related_name='date_patients')
    serial_no = models.CharField(max_length=255, blank=True, null=True)
...
class TextModel(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    hospital = models.ForeignKey(Hospital_model, on_delete=models.CASCADE, related_name='text_hospital')
    text=models.TextField(null=True, blank=True)
class Outbound_assistant(models.Model):
    id=models.UUIDField(primary_key=True,default=uuid.uuid4)
    hospital = models.ForeignKey(Hospital_model, on_delete=models.CASCADE, related_name='hospital_patients')
...
class Outbound_Hospital(models.Model):
    ...
    calling_process=models.CharField(max_length=30,choices=CALLING_STATUS_CHOICES,default="not_happened",null=True,blank=True)
    def __str__(self):
...
class EscalationModel_inbound(models.Model):
    ...
    patient = models.ForeignKey(
        'Inbound_Hospital',  # Adjust app/model name as needed
        on_delete=models.CASCADE
    )
    issue_description = models.TextField()
    status = models.CharField(
