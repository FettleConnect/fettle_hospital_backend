...
class PdfView(APIView):
    ...
    def post(self,request):
        try:
            ...
            total_revisits = q_visits.values('hospital', 'mobile_no').annotate(visit_count=Count('id')).filter(visit_count__gt=1).count()
            
            revisit_conversion_rate = (total_revisits / unique_patients * 100) if unique_patients else 0
            connected_people_rate = np.round((call_cc/connected_data)*100, 2) if connected_data else 0
            community_members_rate = np.round((community_members/connected_data)*100, 2) if connected_data else 0
            
            dict_obj={
                '{{reporting_period}}': f"{start_str} to {end_str}", '{{hospital_name}}': hospital_name.title(),
                '{{call_patients}}': connected_data, '{{call_answer_rate}}': connected_people_rate,
                '{{community_added}}': community_members, '{{community_conversion_rate}}': community_members_rate,
                '{{poll_number}}': poll_participants, '{{escalation_number}}': total_escalations,
                '{{revisit_conversion_rate}}': revisit_conversion_rate, '{{revisit_number}}': total_revisits,
                '{{call_connected}}': call_cc,
                '{{call_p}}': connected_data, '{{calla_c}}': connected_people_rate,
                '{{com_c}}': community_members, '{{coma_c}}': community_members_rate,
                '{{ess_c}}': total_escalations, '{{reva_c}}': revisit_conversion_rate,
                '{{rev_c}}': total_revisits, '{{call_c}}': call_cc
            }
            ...
            else: template = "Amor-Hospitals-May-2025-PTS-Report.docx"
        ...
        try:
            ...
            return Response({
                "roi_financial": [
                    ...
                    {"name": "Revenue Influenced", "value": booked * 650, "unit": "₹"},
                    {"name": "Leakage Prevented", "value": int(missed * 0.42 * 650), "unit": "₹"}
                ],
                "roi_efficiency": [
                    {"name": "Staff Hours Saved", "value": np.round(tdur/60, 1), "unit": "hrs"},
                    {"name": "Equivalent FTE Freed", "value": np.round(tdur/6000, 2), "unit": "FTE"},
                    {"name": "Cost Efficiency Value", "value": int(tdur/6000 * 40000), "unit": "₹"}
                ],
                "error": 0
            })
        except Exception as e: return Response({"error": 1, "msg": str(e)})
...
class DepartmentAnalytics(APIView):
    ...
    def get(self, request):
        try:
            ...
            cd = request.query_params.get('call_direction', 'outbound')
            if cd == 'inbound':
                d_qs = Inbound_Hospital.objects.filter(hospital_id=user_id).values('department').annotate(i=Count('id'))
                f_qs = CallFeedbackModel_inbound.objects.filter(patient__hospital_id=user_id)
                df = 'department'
            else:
                d_qs = Patient_model.objects.filter(hospital_id=user_id).values('department').annotate(i=Count('id'))
                f_qs = CallFeedbackModel.objects.filter(patient__hospital=user_id)
                df = 'patient__department'
            stats = f_qs.values(df).annotate(b=Count('id', filter=Q(call_outcome='positive')))
...
class CampaignView(APIView):
    ...
    def get(self, request):
        try:
            ...
            data = [{
                "id": str(c.id), "name": c.name, "status": c.status, "created_at": c.created_at,
...
class MediVoiceSessionView(APIView):
    ...
    def post(self, request):
        try:
            ...
            d = request.data
            s = MediVoiceSession.objects.create(doctor=doctor, patient_name=d.get('patientName'), patient_mobile=d.get('patientMobile'), overall_summary=d.get('overallSummary'))
            for t in d.get('transcriptions', []):
                MediVoiceTranscription.objects.create(session=s, speaker=t.get('speaker'), text=t.get('text'), timestamp=t.get('timestamp', 0.0))
...
class DoctorTranscriptionView(APIView):
    ...
    def get(self, request):
        try:
            ...
            data = [{
                "id": str(s.id), "doctorName": s.doctor.name, "patientName": s.patient_name, "overallSummary": s.overall_summary, "createdAt": s.created_at,
