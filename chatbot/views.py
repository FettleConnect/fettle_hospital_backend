...
from .models import DermatologyPatient, DermatologyThread
...
def _get_active_thread(patient: DermatologyPatient) -> DermatologyThread:
    ...
    thread = (
        DermatologyThread.objects.filter(patient=patient, status="active")
        .order_by("-created_at")
        .first()
    )
    if not thread:
...
class ChatView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            if thread_id:
                try:
                    thread = DermatologyThread.objects.get(
                        id=thread_id, patient=patient
                    )
    ...
    def post(self, request):
        ...
        try:
            ...
            question = request.POST.get("question", "") or request.data.get(
                "question", ""
            )
            thread_id = request.POST.get("thread_id", "") or request.data.get(
                "thread_id", ""
            )
            ...
            if images:
                ...
                existing_urls: set[str] = {
                    u for m in conv for u in (m.get("images") or [])
                }
            ...
            if thread.mode == "post_payment_intake":
                ...
                if "INTAKE COMPLETE" in question and "Summary:" in question:
                    ...
                    t = threading.Thread(
                        target=_trigger_draft_generation, args=(thread,)
                    )
                    t.daemon = True
                    t.start()

                    return Response(
                        {"result": "", "role": "ai", "mode": "dermatologist_review"}
                    )
...
class ConsultationListView(APIView):
    ...
    def get(self, request):
        try:
            patient = DermatologyPatient.objects.get(email=request.email)
            threads = DermatologyThread.objects.filter(patient=patient).order_by(
                "-created_at"
            )
            history = [
                {
...
class ArchiveConsultationView(APIView):
    ...
    def post(self, request):
        try:
            ...
            DermatologyThread.objects.filter(patient=patient, status="active").update(
                status="completed"
            )
            # Create new thread
...
class DoctorChatAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            threads = (
                DermatologyThread.objects.filter(payment_status="paid")
...
class DoctorSendResponseView(APIView):
    ...
    def post(self, request):
        try:
            conversation_id = request.POST.get("id", "") or request.data.get("id", "")
            message = request.POST.get("question", "") or request.data.get(
                "question", ""
            )
