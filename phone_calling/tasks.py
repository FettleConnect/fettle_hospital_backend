...
def process_outbound_calls(json_payload):
    try:
        ...
        if transcript_res["error"] == 0 and metadata_res["error"] == 0:
            ...
            recording_url = f"s3://{settings.LIVEKIT_BUCKET_NAME}/video_record/{vapi_id}.ogg"
            ...
            json_response = json_audio(patient_id, text_message, started_at, call_duration)
            
            if "error" not in json_response and json_response.get('call_status') == 'connected':
                # Sync Feedback
                requests.post(f"{url_backend}/api/callfeedback/", headers=headers_url, json=    {
                    ...
                    "escalation_required": json_response.get("escalation_required", False),
                    "patient_id": patient_id,
                    "remarks": json_response.get("remarks", ""),
                    "revisit_encouraged": json_response.get("revisit_encouraged", False)
                })

                if json_response.get('escalation_required'):
                    requests.post(f"{url_backend}/api/escalationfeedback/", headers=headers_url, json=    {
                        "patient_id": patient_id,
                        "issue_description": json_response['remarks'],
                        "department": Patient_model.objects.get(id=patient_id).department
                    })
                    cloudconnect_whatsapp_msg(f"Escalation for {mobile_no}: {json_response['remarks']}")
