...
def run_lk(cmd_list):
    full_cmd = ["/usr/local/bin/lk", "sip"] + cmd_list + ["--url", "http://localhost:7880", "--api-key", "API5Jmt5us6g875", "--api-secret", "ZPxu6uKWuj7KLpK16j2lYQ9FFFy9ESqHDXEUXdmDafR"]
    return subprocess.check_output(full_cmd).decode().strip()

# Cleanup
for rule_id in ["SDR_cFXaRZYBa85A", "SDR_siswx5uKU3bh", "SDR_t3wHgKpiW5gp"]:
    try: run_lk(["dispatch", "delete", rule_id])
    except: pass
...
try:
    ...
    with open("/tmp/disp.json", "w") as f: json.dump(dispatch, f)
